...
# ==========================================
# SIMPLIFIED WRAPPER FUNCTIONS FOR BOT
# ==========================================

# Global payment instance (akan diinisialisasi dari config)
_payment_instance = None

def initialize_payment(server_key: str, is_production: bool = False):
    """Initialize global payment instance"""
    global _payment_instance
    _payment_instance = MidtransPayment(server_key, is_production)
    return _payment_instance

def get_payment_instance():
    """Get or create payment instance"""
    global _payment_instance
    if _payment_instance is None:
        # Import config here to avoid circular import
        try:
            import config
            _payment_instance = MidtransPayment(
...
def create_payment(user_id: int, amount: int, customer_details: dict = None):
    """
    Simplified wrapper untuk create payment
    ...
    """
    try:
        # Generate order ID with TOPUP prefix for top-up transactions
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        order_id = f"TOPUP-{user_id}-{timestamp}"
        
        # Get payment instance
        payment = get_payment_instance()
        
        # Create transaction
        result = payment.create_qris_transaction(
            order_id=order_id,
            amount=amount,
            customer_details=customer_details
        )
        
        if result and result.get('transaction_status') == 'pending':
            # Extract QR URL
            qr_url = None
            ...
            error_msg = "Failed to create payment"
            if result:
                error_msg = result.get('status_message', error_msg)
...
def get_payment_status(order_id: str):
    """
    Simplified wrapper untuk check payment status
    ...
    try:
        ...
        # Check status
        result = payment.check_transaction_status(order_id)
        ...
        if result:
            ...
            return {
                ...
                'transaction_status': transaction_status,
                'fraud_status': fraud_status,
                'amount': int(result.get('gross_amount', 0)),
        ...
        print(f"❌ Exception in get_payment_status: {e}")
        import traceback
        traceback.print_exc()
        return {
            'success': False,
...
def cancel_payment(order_id: str):
    ...
    try:
        payment = get_payment_instance()
        result = payment.cancel_transaction(order_id)
